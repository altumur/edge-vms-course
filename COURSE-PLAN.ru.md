...
Поставляемая VMS на краю — это семь слоёв в глубину. Курс строит их в порядке зависимостей, по модулю на слой, и каждый заканчивается чем-то работающим.
...
### М9 — EdgeVMS, уроки 5–9: Postgres и воркер · [написан](./М9_EdgeVMS/README.md)

*Свёрнуто в М9 12 сентября 2026: пять уроков рекордера идут за четырьмя уроками прибора, так что один модуль ведёт коробку от A/B-корня до базы, которая владеет тем, чем коробка должна быть. Проектная записка — часть 2 файла [`module-design.md`](./М9_EdgeVMS/module-design.md).*

Спецификация облачной VMS запрещала базу напрямую. Прибору она нужна, и понимание того, *почему ответ перевернулся*, — половина модуля: в облаке конфигурацию держал KVS; на земле её держит коробка. Другая половина — в том, что строка, говорящая, что камера должна писаться, остаётся желанием, пока что-нибудь не сделает её истиной.

- Схема для камер, сайтов и хранения; миграции как поставляемый артефакт, который запускается при загрузке на коробке, к которой никто не приходит
- **Колонки, принадлежащие оператору, против колонок, принадлежащих контроллеру** — различение, которое не даёт размещению рекордеров попасть в руки оператора
- **Самое критичное:** `PGDATA` живёт на разделе данных, поэтому переживает A/B-обновления ОС нетронутым. Это трёхсторонняя граница М9 с настоящими последствиями
- Цикл сверки, сначала построенный против поддельного актуатора: желаемое сохранено, фактическое выведено, `observed_revision >= revision` как единственная проверка применённости
- Пятьдесят конвейеров GStreamer в одном процессе Python — граница GIL показана, `watchdog` для обнаружения залипаний, и где Python перестаёт быть верным ответом

### М10 — ServerVMS: форма платформы на одном сервере · 5 уроков · [написан](./М10_ServerVMS/README.md) · [проект](./М10_ServerVMS/module-design.md) · [код](./М10_ServerVMS/vmsserver/README.md)
...
- **`clustervms/` построен на `vmsserver/` из М10**, импортируя контракт, контроллер, воркер и архив без изменений; `vmsserver-go/` и `clustervms-go/` — оба модули на Go с тем же отношением (директива `replace` вместо `sys.path`), 70 тестов и измерение
...
- **Сроки жизни против терпимости к офлайну:** *терпимый простой = срок жизни сертификата − запас на обновление*. Отзыв на краю — задача про срок жизни, а не про список
...
**39 уроков**, то есть полный семестр. Число менялось трижды: вниз с 49, когда свёртывание слоя над доменом убрало четыре урока избыточности; вверх на один, когда система лицензий, бывшая пунктом списка, оказалась уроком; и вниз на семь, когда пятнадцать коротких уроков М8 слили в восемь — его первые двенадцать стали пятью многочастными уроками, по одному на исходный подмодуль, а последние три остались как были. **Каждый модуль нумерует свои уроки с 1**; ссылка в другой модуль всегда несёт модуль: *М9, урок 6*, никогда просто номер. М9–М14 — каждый настоящий модуль, а не приложение.
