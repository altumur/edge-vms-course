...
### М10 — ServerVMS: форма платформы на одном сервере · 5 уроков · [написан](./М10_ServerVMS/README.md) · [проект](./М10_ServerVMS/module-design.md) · [код](./vmsserver/README.md)
...
- **`clustervms/` построен на `vmsserver/`**, импортируя контракт, контроллер, воркер и архив без изменений; `vmsserver/worker/` — половина того же пакета на Go: два воркера, на кластере запускаемые как работы, против Python-контроллера, с которым они встречаются в хранилище
