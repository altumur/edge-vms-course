...
| **Live gateway** | The cluster-level job that serves pictures to browsers — WebRTC live, fMP4 playback, TURN, transcoding — subscribing to a worker's live tee once per camera and fanning out from there. Placed by constraint (GPU, public address), never by hostname. A worker's only viewer, together with the console; a worker never serves a browser. (М12, *Who serves browsers*) |
