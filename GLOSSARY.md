...
| Term | Meaning here |
|---|---|
| ~~**recorder**~~ | *Retired (12 September 2026).* М9 Lessons 5–9's recorder: a process with its own database, cameras and archive index, distinct from the box it ran on. Under М10's shape what is on a server is the platform's stores, a *resource* on its disks and the *workers* placed there — and their sum is the *server*, so the fifth word went. Appears in М9 (history) and in the preserved earlier revisions of М12's [`where-the-database-lives.md`](./М12_DomainVMS/where-the-database-lives.md). |
| **Event** | An observation — a detection, a silence, an operator's mark — written by the worker holding a unit's epoch into that unit's bucket on its server's resource: `<subsystem>/<unit>/e<epoch>/<start>Z.events.jsonl`, recording or not. A platform piece any subsystem uses; the VMS's buckets sit beside its footage. Closed into the manifest, fenced by the path, retained by their own days. No controller writes events; each resource process keeps an `EventDatabase` over its own — a cache — and the console merges them (М10 L3 and L9, М11 L3). |
| **Worker** | A DriverPack shard with N cameras assigned — holds the pipeline and routes it; stable identity, movable between servers, stateless but for a spool. What М9's recorder becomes under М11's decision *2c*. 1+ per cluster, by workload. |
| **Resource** (platform job) | One per server, pinned: the platform's `resource` job (`w2cplatform/resource.py`) heartbeats what is on the server's disks under `platform/resources/<server>`, serves every subsystem's buckets, mirrors closed ones to the next resource when `platform/mirror` is on, restores them after a replaced disk, retains by each subsystem's `<sub>/retention` rows, and runs the passes subsystems register on it (the VMS: manifests and media). |
...
| **Recorder** | A worker of the `rec` subsystem (М10 Lesson 5), the only one placed on top of the archive: its unit is one camera's *recording* (`rec/recordings/<cam>`, an explicit row — the page's *Record*), its capacity is what its server's disks take. It holds no camera: it subscribes to the worker's tee — shared memory (`live_shm`) when they share a server, the RTSP fan-out (`live_url`) otherwise — and writes `rec/<cam>/e<epoch>/` on its server's resource under its own epoch; `requires: resource`, `servers: distinct` by default, and `home` on its row — the server whose archive holds it. It follows nothing: it is the one thing here pinned to disks, so the camera follows it (`near: rec`) and not the other way round. |
| **Live gateway** | A worker of the `live` subsystem (М10 Lesson 8): its unit is a camera's fan-out, its capacity is viewers. It subscribes to a worker's RTP tee once per camera and fans out to N browsers over WebRTC (WHEP signalling through the console); the fan-out unit is created by the first viewer and deleted by the gateway after a grace period. `count = N` by viewer headroom; placed by constraint (a public address, a GPU for transcoding), never by hostname. A worker's only viewer, together with the console; a worker never serves a browser. М12 adds TURN, the token check and placement across clusters (*Who serves browsers*). |
| **Console** (cluster) | The cluster-level job that serves the UI: static files, the API façade, the read model built from the workers' heartbeats and the controller's snapshot, TLS, token verification. Stateless; at the domain, proxies edits to the owning cluster's console, whose grants decide. The domain cluster runs the same console pointed at every cluster. Distinct from М9's recorder console (М9 Lesson 9), which serves only these two jobs. Since М10 Lesson 7 it is its own job — one on every server, nothing in front — with its own token — the operator's rows, never placement — and, like the controller, it is the platform's `SpecConsole` run from the subsystem's YAML; a subsystem adds only the routes that are its own (the VMS: the timeline and the bytes). |
...
| ~~**Domain controller**~~ | **Retired.** Named a component the design dissolved: its database became cluster Variables and object stores, its scheduler became Nomad, its API moved onto every cluster's console, its restore point dissolved into the cluster's raft. What remains at the domain is a signer, a stateless placement function and a read view — none of them authoritative, which is why the old name misleads. |
...
| Term | Meaning here |
|---|---|
| **Cluster** | Servers close enough to share **a network you would bet recording on** — one LAN, usually one server room. What М11 builds, and the boundary set by **physics**. A worker fails over *within* a cluster and **never across one**, because its footage is on that cluster's disks. |
...
| **Licence** (entitlement) | A signed document from the vendor naming a **domain id** — never a server or a box — with a camera count, features, a validity window and a grace period. Pulled through the domain's update server like a bundle, held in the domain cluster's Variables, verified offline by the domain against a key shipped in the product. Enforced **at admission** (placement refuses a new camera) and never at runtime: a lapsed licence removes the right to grow, not the recording. No revocation — lifetimes only, and a perpetual licence has none. М14 Lesson 3; the consuming side is М12 Lesson 5. |
...
| **Edge / cloud / mixed** | Where a domain's servers physically are — at the site, rented in a cloud region, or both in one system. A placement decision per site, not three products: a worker cannot tell which it is running on. |
...
| **Failover** | Moving work off something that failed. In this course it happens at two levels: the scheduler moves a **worker** to a healthy server; the domain reassigns **cameras** only when an operator rebalances. |
...
| | |
|---|---|
| **Stretch** | The part of a segment given to one reader of the archive: `(segment, from, to)`. It appears when an interval is read backwards — on an operator's request rather than as recording goes — and it is needed because a segment is not good or bad as a whole: where epochs overlap (a zombie and the survivor describe the same minutes) every elementary interval goes to the **highest epoch** that covers it, and one file can come back in two pieces or in none. The stretch is then clipped to the window asked for: the decoder starts at the head of the file, the operator does not (М10B Lesson 20). |
...
| | |
|---|---|
| **Authentication** | Proving *who* a subject is. Federated in М12, so there is one Alice across every cluster and domain. |
| **Authorization** | Deciding what that subject may *do*. Deliberately **not** centralised: grants live in each cluster so they can be checked with the domain unreachable, and carry an expiry that bounds the revocation window. |
| **Secret** (a row's field) | A unit field whose name ends `_secret` — a device password, and anything like one. It has exactly two honest points of contact: the operator writes it through the console, and the worker holding the unit reads it to open the device. Between them it is `***`: `mask_secrets` masks it in the reply to a create (and therefore in the copy kept under the idempotency key), in the reply to an update, and in `GET /<rows>`; a spec that names a secret in its `snapshot` does not load, because the snapshot leaves the cluster. The rule is a suffix declared where the field is, not a registry that would drift from the specs. What it does not buy: the row holds the secret in the clear, and the store's ACL bounds writers, not readers (М10A Lesson 23; М10B Lesson 19). |
| **Blob** (a field type) | A unit field that does not fit in a row: a per-pixel mask, a panel's firmware, a model. The platform never looks inside it. The bytes live in the object store under `<sub>/blobs/sha256-<hex>` and the row keeps a **digest** — and every useful property follows from that one choice: different bytes are a different digest, so a different row, so `revision` moves and the reconciler restarts the unit, which means rotation comes from the mechanism that was already there; a digest key is written once, so last-writer-wins has nothing to decide; a worker can cache by digest forever; the same lump on a hundred units is stored once. The object is written FIRST, the row second. The spec refuses a blob in `snapshot:` at load time and refuses raw bytes in a row at write time, naming the route `PUT /<rows>/<id>/<field>`. What it does not buy: nothing sweeps orphaned blobs — `blobs_referenced()` says what to keep, but nothing deletes (М10A Lesson 26). |
| **Blob sweep** | The one place the platform deletes an object. Needed because a blob key is content addressed: every edit of a `blob` field makes a new permanent object, whereas a heartbeat's key is reused by the slot (and its stale version is load-bearing — a worker measures its own failover from it). Two passes: **mark** writes the digests no row names into `<sub>/sweep` and deletes nothing; **sweep**, not before the grace period, re-checks them, CLEARS THE ROW BY CAS and only then removes the objects. The grace comes from the structure — a blob created between the passes is not on the list — so no object timestamps are needed, and `variables://` has none. The order is the whole safety argument: a lost CAS costs nothing while nothing has been deleted, and `put_blob` takes its digest off the list precisely so that CAS is lost. Bounded per pass, because its own bookkeeping is a row under the store's ceiling (Lesson 26). Run by the console: `blobs/` is the console's, and `destroy` is granted on that prefix alone (М10A Lesson 29). |
| **Store ceiling** (*max_bytes*) | How much a store holds in one object or under one path — a property the store DECLARES, not a number to remember. `0` means no ceiling; a Nomad Variable's is 65536 over keys and values together. A write over it is `TooLarge`: refused, never truncated, because a store that drops the tail is a store that lies about `get`, and every CAS loop is built on `get` telling the truth. The refusal reaches a person: 413 on an operator's edit, "how many units on which worker" on a snapshot shard. Clause 8 of the contract, run against any backend; `memory://<name>?max_bytes=n` makes it observable without a cluster (М10A Lesson 26). |
