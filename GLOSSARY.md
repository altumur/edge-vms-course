...
| **Worker** | A DriverPack shard with N cameras assigned — holds the pipeline and routes it; stable identity, movable between servers, stateless but for a spool. What the recorder becomes under М11's decision *2c*. 1+ per cluster, by workload. |
...
| ~~**Domain controller**~~ | **Retired.** Named a component the design dissolved: its database became cluster Variables and object stores, its scheduler became Nomad, its API moved onto every recorder, its restore point moved into the cluster. What remains at the domain is a signer, a stateless placement function and a read view — none of them authoritative, which is why the old name misleads. |
...
| Term | Meaning here |
|---|---|
| **Cluster** | Servers close enough to share **a network you would bet recording on** — one LAN, usually one server room. What М11 builds, and the boundary set by **physics**. A recorder fails over *within* a cluster and **never across one**, because its footage is on that cluster's disks. |
...
| **Licence** (entitlement) | A signed document from the vendor naming a **domain id** — never a server or a box — with a camera count, features, a validity window and a grace period. Pulled through the domain's update server like a bundle, held in the domain cluster's Variables, verified offline by every recorder against a key shipped in the product. Enforced **at admission** (placement refuses a new camera) and never at runtime: a lapsed licence removes the right to grow, not the recording. No revocation — lifetimes only, and a perpetual licence has none. М14 Lesson 3; the consuming side is М12 Lesson 5. |
...
| **Edge / cloud / mixed** | Where a domain's servers physically are — at the site, rented in a cloud region, or both in one system. A placement decision per site, not three products: a recorder cannot tell which it is running on. |
...
| **Failover** | Moving work off something that failed. In this course it happens at two levels: the scheduler moves a **recorder** to a healthy server; the domain reassigns **cameras** only when an operator rebalances. |
...
| **Authorization** | Deciding what that subject may *do*. Deliberately **not** centralised: grants live in each recorder so they can be checked with the domain unreachable, and carry an expiry that bounds the revocation window. |
