...
| **Resource** (platform job) | One per server, pinned: the platform's `resource` job (`psimplatform/resource.py`) heartbeats what is on the server's disks under `platform/resources/<server>`, serves every subsystem's buckets, mirrors closed ones to the next resource when `platform/mirror` is on, restores them after a replaced disk, retains by each subsystem's `<sub>/retention` rows, and runs the passes subsystems register on it (the VMS: manifests and media). |
...
| **Recorder** | A worker of the `rec` subsystem (М10 Lesson 5), the only one placed on top of the archive: its unit is one camera's *recording* (`rec/recordings/<cam>`, an explicit row — the page's *Record*), its capacity is what its server's disks take. It holds no camera: it subscribes to the worker's tee — shared memory (`live_shm`) when they share a server, the RTSP fan-out (`live_url`) otherwise — and writes `rec/<cam>/e<epoch>/` on its server's resource under its own epoch; `requires: resource`, `servers: distinct` by default, `near: vms` as an affinity. |
