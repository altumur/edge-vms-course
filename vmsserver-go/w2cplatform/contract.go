...
// RequestKey is `<name>/requests/<id>` — bounded work the OPERATOR asked a worker to do, outside its
// ordinary pass. The shape the blob sweep's row already has, generalised: the console writes it, a
// worker's pass reads it, and the platform never looks inside. What a request MEANS is the subsystem's.
func (s Subsystem) RequestKey(rid string) string { return s.Name + "/" + Requests + "/" + rid }
func (s Subsystem) RequestsPrefix() string       { return s.Name + "/" + Requests + "/" }
...
// Requests: `<name>/requests/<id>` — bounded work an operator asked for, written by the console.
