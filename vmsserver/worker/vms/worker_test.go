...
// boxWithCameras: the rows a console would have written and the assignment bookkeeping a controller
// would do — as ROWS IN THE STORE, which is the whole of what a worker can see of either.
//
// There is no controller in this module on purpose: placement is Python's (../../vms/controller.py), and
// a Go test that set one up would be asserting against a second implementation of a policy this half does
// not own. What the worker's contract actually says is "given this row and this assignment, it does X",
// and that is what these tests now state. p.Controller below is not placement — it is the contract's own
// bookkeeping (slots, assignments, epochs), the same rows the Python controller writes.
func boxWithCameras(t *testing.T, n int) (*testbox.Box, *p.Controller) {
	t.Helper()
	box := testbox.NewBox()
	for i := 1; i <= n; i++ {
		setCamera(t, box, strconv.Itoa(i), p.Items{"id": strconv.Itoa(i), "name": "cam" + strconv.Itoa(i),
			"source": "driverpack://file/cam" + strconv.Itoa(i) + ".mp4", "revision": "1"})
	}
	return box, p.NewController(vms.VMS, box.Vars, box.Objects, box.Wall.Now)
}

// setCamera writes vms/cameras/<id> the way the console does — and a bumped `revision` is the only way an
// edit reaches a worker, which is why the tests below edit by writing one.
func setCamera(t *testing.T, box *testbox.Box, id string, items p.Items) {
	t.Helper()
	_, idx, _ := box.Vars.Get("vms/cameras/" + id)
	if _, err := box.Vars.Put("vms/cameras/"+id, items, idx); err != nil {
		t.Fatal(err)
	}
}
...
func TestWorkerRunsItsAssignmentAndTakesAnEpochPerCamera(t *testing.T) {
	...
	setCamera(t, box, "1", p.Items{"id": "1", "name": "gate", "source": "driverpack://file/cam1.mp4", "revision": "2"}) // an edit: revision 2
...
func TestAReassignmentIsNotAZombie(t *testing.T) {
	...
	w1 := worker(t, box, "w-1", vms.NewFakeActuator(), vms.VmsWorkerOptions{})
	w1.ReconcileOnce()
	ctl.Assign("w-1", nil) // the move, as the worker sees it: two assignment rows, and no word to anyone
	ctl.Assign("w-2", []string{"1"})
