...
- **Урок 4** — `Subsystem`, `Assignment`, `Heartbeat`: чем эти классы обмениваются.
- **Урок 5** — эпоха и аренда: воркер их здесь соединяет.
- **Урок 6** — слот: три метода захвата уже написаны и лежат в этом же классе.
...
Тот же цикл, что в `next_epoch` из урока 5, но вынесенный в метод и принимающий **функцию изменения**. Дальше в курсе через него пойдёт почти каждая запись: назначение, размещение, строки оператора, политики, производные строки. Разберём три решения.
...
`released_slots()` и `retire()` разобраны в уроке 6; здесь достаточно заметить, что `retire` — единственный метод контроллера, который пишет **слот**, и это не противоречит «слоты захватывают воркеры»: захват — воркеров, освобождение снаружи — операторское.
...
`name` может быть `None` до захвата (урок 6). Именованный воркер (`%i`) получает имя сразу и всё равно проходит через захват — просто с предпочтением.
...
Четыре строки, соединяющие два механизма урока 5: взять номер и **тут же** завести на него аренду. Раздельно их не берут нигде — эпоха без аренды была бы правом без срока.
...
Сумма конфликтов по всем арендам — то, что уходит в heartbeat и в метрику. И сама публикация: `**extra` принимает что угодно, потому что платформа это не разбирает (урок 3).
...
5. Сделайте эпоху одной на воркера вместо одной на единицу. Какие два случая перестанут различаться (подсказка: урок 5, шаг 11)?
...
Платформа закончена как механика. Дальше начинается то, ради чего она такая: [**урок 8**](08-SubsystemSpec.md) читает десять строк YAML в датакласс — и с этого момента контроллер подсистемы перестаёт быть кодом, который кто-то пишет, и становится файлом, который кто-то заполняет.
