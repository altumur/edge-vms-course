# Урок 13 — Ресурс как работа платформы: хранение, зеркало, ватерлиния и две ОС
...
- **Урок 11** — бакеты, `buckets_under`, `subsystems_under`: то, из чего ресурс узнаёт, что у него лежит.
- **Урок 12** — база событий: ресурс её содержит и сообщает ей об удалениях.
- **Урок 3** — heartbeat: у ресурса он свой, под своим префиксом.
- **Урок 2** — `FileVariables` и сериализация записи блокировкой файла `<root>/lock`.
- **Урок 10** — `register_constraint`: та же дверь «код под именем», что откроется здесь; и предпочтение против фильтра — тот же ход мысли повторится в ступенях освобождения.
...
Форма — и ничего о содержании. Ресурс знает, что подсистемы кладут своё под своим именем, что внутри — единицы, эпохи и бакеты (урок 11), и что рядом бывает что-то ещё, чего он не трогает. В М10B рядом с бакетами камеры лежат видеофайлы и манифест; ресурс их не понимает и не должен.
...
Три уровня, от частного к общему. Строка этой единицы — производная строка из урока 8, написанная контроллером подсистемы. Строка подсистемы — умолчание для всех её единиц. И год, если не сказано ничего.

Год как последнее умолчание — выбор в пользу **сохранения**. Ошибка в конфигурации приведёт к тому, что данные проживут дольше нужного (дорого, но поправимо), а не к тому, что они исчезнут (непоправимо). То же соображение, что и с удалением строк в уроке 9: пометить, а не стереть.
...
`server`, `ts` — кто и когда; отсюда `resource_state` контроллера из урока 10 берёт свои три состояния. `url` — куда стучаться: по нему консоль спрашивает события, а соседи кладут копии. **Адрес ресурса нигде не настраивается — он объявляется.**

`usage` — сколько занято байт: страница показывает, оператор смотрит. `units` — какие подсистемы и единицы здесь лежат (`subsystems_under` из урока 11); это же поле в М11 отвечает на вопрос, у кого искать записи камеры 7.
...
**Только закрытые** — те, чей промежуток кончился. Открытый бакет ещё дописывается (урок 11), и копировать его пришлось бы снова и снова, каждый раз целиком.
...
Кого спрашивать — видно из чужих heartbeat'ов: `self.server not in hb["mirrors"]` пропускает тех, у кого моих копий нет. Что уже есть — не тянется (`if os.path.exists`). Запись — через `.tmp` и `os.replace`, потому что тянется по сети и может оборваться (урок 3).
...
Одна строка, и это третья дверь «код под именем» в курсе — после `register_constraint` (урок 10) и подсистем вообще. Объект должен уметь `pass_(now) -> dict`; больше от него не требуется ничего.
...
Пять веток. Первые две — межресурсный разговор (зеркало). Третья — то, что спрашивает консоль (урок 12). Четвёртая — выдача файла: по ней сосед тянет копию, и по ней же отдаются копии обратно владельцу.
...
Ресурс без базы честно отвечает 503 с объяснением, а не пустым списком. Тот же принцип, что `catching up` в уроке 12: **не отвечать коротко.**
...
- Все на полу и места нет — это число в отчёте, а не тихий рез: тот же выбор, что «разместить негде» в уроке 10.
...
Платформа умеет всё, кроме одного: показать человеку. [**Урок 14**](14-SpecConsole.md) строит консоль из той же спецификации, из которой построен контроллер, — и первое, что делает, — отдаёт странице описание подсистемы, чтобы та собрала себя сама.
