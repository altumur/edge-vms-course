# Урок 14 — Консоль как данные: чтения, записи и `Mount`
...
У платформы есть всё, кроме экрана. И вопрос, с которого этот урок начинается, тот же, что в уроке 8: **четыре подсистемы — четыре консоли?**
...
- **Урок 8** — спецификация: поля, типы, `refuse`, два ACL; почему ошибка поля приходит именно оттуда.
- **Урок 9 и 10** — контроллер: `create`/`update`/`delete`, `read_model`, `unplaceable`, `policy`/`set_policy`, `POLICY_CHOICES`, `resource_state`, `idle_by_policy`.
- **Урок 12** — `MergedIndex`: то, чем консоль отвечает про события.
- **Урок 3** — heartbeat и `extra`: `/servers` и `/metrics` целиком построены на нём.
...
Консоль принимает **контроллер**, а не хранилище. Тому есть причина, и она про права: контроллер, переданный сюда, построен на дескрипторе с **токеном консоли** (урок 8), то есть умеет писать строки оператора и не умеет писать размещение. Тот же класс `SpecController`, другой токен — и консоль физически не может сделать того, чего ей нельзя. Не «не вызывает метод», а получает `Forbidden`.

`index` — то, чем отвечать про события: `MergedIndex` из урока 12. Может быть `None` — тогда `/events` честно отвечает 503.
...
Оператор нажал «отметить момент». Это наблюдение — ровно в том смысле, в каком наблюдения определены в уроке 11: нечто, замеченное в некоторое время. Куда его писать?

Соблазн: в бакет той камеры, о которой отметка. Нельзя. У бакета один писатель, определённый эпохой (урок 11), и эпоху камеры держит **воркер**, а не консоль. Консоль, пишущая в чужой бакет, — это второй писатель, то есть ровно то, от чего вся конструкция защищается.
...
А связь с камерой — через **поле** `cam` внутри события (урок 12, шаг 2), а не через путь. Точно так же поступает детектор в М10B: его событие о камере 7 лежит в его бакете и несёт `cam: 7`.
...
Вот что здесь важно осознать. Страница **не знает** ни одной подсистемы: ни камер, ни тиков. Она умеет отрисовать «подсистему вообще» — и потому одна и та же на все четыре. В уроке 15 это будет видно целиком; сейчас достаточно, что описание содержит ровно то, из чего экран собирается.
...
Уборка старше суток, не чаще раза в минуту на процесс, **по пути** — внутри `claim`. Без отдельного потока и без задачи по расписанию: то, что делается редко и дёшево, можно делать попутно. Монотонные часы для интервала, настенные для возраста заявки (урок 5).
...
`rows` — модель чтения (урок 10): что **наблюдается**, собранное из heartbeat'ов воркеров, с возрастом и состоянием. `configured` — что **настроено**, то есть строки из хранилища.
...
`worker` и `reason` — из строки размещения: что решил контроллер и почему (урок 10). `directory` — из скана назначений: в чьих списках она сейчас числится.
...
И последняя строка — причина, по которой ответов два. Во время перемещения (урок 10, `move`) единица недолго числится **у обоих** воркеров, и `where` вернёт `"w-1+w-2"`. Строка размещения при этом уже называет нового. Оператор, спросивший в эту секунду, увидит оба ответа и поймёт, что происходит переезд, — вместо того чтобы получить один ответ и удивляться, почему воркер не совпадает.
...
Ресурсы — их heartbeat'ы (урок 13) с добавленным состоянием. Опять: **молчащие показываются**, помеченные, а не скрываются. Та же линия, что модель чтения с её `stale` и ответ базы с `catching up`.

`/unplaceable` — отдельный вопрос про единицы, которых никто живой не может взять (урок 10).
...
Вторая строка — самая плотная в файле, и она решает задачу из урока 12: база не знает текущих эпох, их приносит спрашивающий.
...
Ключ разбирается из пути: `det/epoch/7-linecross` → `("det", "7-linecross")`. Никакого знания о подсистемах — разбор строки по известной раскладке (урок 4).
...
`{**r, "worker": None}` — та самая честность. Контроллер вернул строку такой, какой она легла в хранилище: с `id`, `revision`, полями по умолчанию. Поля `worker` в ней нет вовсе — размещение живёт в другом ключе (урок 4). Консоль дописывает его явным `None`, чтобы страница не гадала: единица есть, воркера у неё пока нет, приходите через секунду.

`Refused` → `400`. Исключение приходит из `refuse` (урок 8): оператор прислал поле, которого нет в спецификации, или значение не того типа, или нарушил ограничение. Это ошибка клиента, и текст в ней уже человеческий — его не надо переводить, его надо передать.
...
`int(body["cam"])` — не придирка к типу. Это поле, **по которому база событий соединяет** (урок 12): отметка оператора должна лечь на таймлайн камеры рядом с детекциями, а соединение идёт по числу. Строка `"7"` и число `7` не встретятся.
...
**Реестра машин нет.** Список серверов получается перечислением того, что билось: `hb.extra["server"]` — строка, которую воркер положил в свой heartbeat (урок 3). Сервер, с которого никто не бился, в ответе не появится, и это правильно: с точки зрения системы его нет.
...
`idle_by_policy` из урока 10: при `servers: distinct` единицы несёт один воркер на сервер, остальные живы и пусты. Без этого флага оператор видит воркер с нулевой нагрузкой и решает, что тот сломан. Флаг переводит «пустой» в «пустой по правилу».
...
`placeable`/`why` — то же правило, по которому решает `_pool` (урок 10), пересчитанное для показа. Дублирование логики? Да, и намеренное: контроллер отвечает решением, консоль — объяснением, и объяснение должно существовать до того, как решение понадобится. Если правило меняется, оно меняется в двух местах — и в уроке 10 у него один тест на оба.
...
`headroom_from` — имя поля heartbeat'а, взятое из YAML (урок 8). Платформа не знает, что у камер запас считается в мегапикселях, а у счётчика в тиках; она знает, что поле называется так, как сказано в спецификации.
...
Единственная метрика, которая считается по **всем** heartbeat'ам, а не только по живым: конфликты эпох — это история, и умерший воркер свой счёт уже не поправит, но рассказать о нём успел. Ненулевые конфликты означают, что двое держали одну единицу и отсечение сработало — то, ради чего написан урок 5.
...
`worst_failover` — арифметика из урока 10, посчитанная один раз при сборке консоли: худший разрыв между смертью воркера и подхватом его единиц. Величина проектная, а не наблюдаемая, и на графике она — линия, с которой сравнивают наблюдаемое.
...
Две ошибки, два кода: `Refused` — 400 (администратор выбрал значение не из `POLICY_CHOICES`), `Forbidden` — 403 (токен не имеет права на `<sub>/policy`, урок 8). Первое — «вы не так написали», второе — «вам нельзя». Свалить их в один код означало бы скрыть от администратора, что дело в правах.
...
11. Снимите флаг `idle_by_policy`. Поставьте `servers: distinct` на трёх воркерах и опишите экран глазами оператора, который не читал урок 10.
...
Порт отвечает, маршруты есть, JSON правильный — а смотреть не на что. [**Урок 15**](15-HTML.md), последний в модуле, разбирает 666 строк `console.html`: список и формы, собранные из `/spec`; таймлайн на канве с метками событий; плеер, просящий файл диапазонами; переключатели оператора и администратора — страница, которая ничего не знает ни об одной подсистеме и показывает любую.
