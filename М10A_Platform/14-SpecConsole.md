...
Две ошибки, два кода: `Refused` — 400 (администратор выбрал значение не из `POLICY_CHOICES`), `Forbidden` — 403 (токен не имеет права на `<sub>/policy`, урок 2). Первое — «вы не так написали», второе — «вам нельзя». Свалить их в один код означало бы скрыть от администратора, что дело в правах.
