...
**Вы напишете:** `psimplatform/resource.py` целиком — heartbeat, сроки хранения по строке каждой подсистемы, зеркало к соседу без всякой карты, возвращение домой после замены диска, регистрацию чужих проходов и HTTP. Единственный процесс курса, у которого нет контроллера.
**Время:** ~90 минут.
