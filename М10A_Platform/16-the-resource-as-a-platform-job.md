...
А у прохода этого урока есть продолжение, до которого мы дойдём в конце модуля. `retain` удаляет по сроку — и это обещание оператору, выполнимое ровно до тех пор, пока хватает места. [**Урок 21**](21-when-the-disk-fills.md) дописывает то, что происходит, когда его выполнить нельзя.
