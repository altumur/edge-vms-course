...
**Вы напишете:** первую половину `w2cplatform/console.py` — `SpecConsole`, собранную из той же спецификации, что и контроллер; идемпотентные ключи, которые живут в хранилище, а не в памяти процесса; и чтения: описание подсистемы, модель чтения, «где эта единица», события.
**Время:** ~90 минут.
