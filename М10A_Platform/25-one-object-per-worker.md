...
И ломалось это тихо. `publish_snapshot()` стоял последним в общем `try` цикла контроллера: размещение к этому моменту уже отработало, в лог уходило «placement pass failed» — про единственное, что **не** упало, — а домен М12 продолжал отдавать вчерашний снимок. Починено в **уроке 28**, и до него эта тишина — часть задачи.
