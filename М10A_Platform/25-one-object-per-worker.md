...
Потолок у хранилища есть, и мы про него знаем — но нигде в коде он не **объявлен**. `FsObjectStore` не падает никогда, поэтому в курсе этот дефект не воспроизводится: его пришлось считать руками. Следующий шаг — сделать предел свойством, которое хранилище о себе сообщает, и превратить «не влезло» из сюрприза в проде в отказ с числом камер в сообщении.
