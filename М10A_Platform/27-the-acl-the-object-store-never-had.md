...
Модуль закончен на том, с чего М11 начнёт всерьёз: правило, выведенное из кода, и файл, который ему обязан соответствовать. Остаток записан честно — уборки осиротевших блобов по-прежнему нет, `publish_snapshot()` по-прежнему стоит в общем `try` с размещением, и воркер по-прежнему может написать heartbeat соседа. Ни одно из трёх не притворяется решённым.
