...
**`folders`** — папки камеры: список путей, уровни через `/` — `Объект А/Подъезд,Периметр`. Камера может лежать в нескольких папках и видна в каждой. Отдельный ключ, и не ради аккуратности.

`labels` **сравниваются** размещением с тем, что сервер о себе объявил, а объявляет он там два разных рода фактов: какие сегменты сети видит (`vlan:cctv-a`) и что у него есть (`gpu` — этим пользуется детектор, урок 14). Положите папку в это множество — и камера станет неразмещаемой, пока какой-нибудь сервер не заявит, что он «видит Подъезд». Абсурд, который приходит под видом сбоя размещения, то есть в самом дорогом виде.

Приставки (`loc:`) нет намеренно. Она понадобилась бы только затем, чтобы протащить папки через поле, которое сравнивают; со своим ключом она не несёт информации, а приставка без информации — это то, что пишут по-разному (`Loc:`, `loc :`) и что некому отказать.

**Папки нет нигде в хранилище — она и есть путь на камерах.** Отсюда два следствия, и оба стоит принять осознанно. Пустых папок не бывает: папка существует ровно до тех пор, пока в ней лежит хоть одна камера. А переименование папки — это переписывание пути у **каждой** камеры под ней. Взамен нет второй сущности, которую надо держать согласованной с камерами: дерево — это то, что его рисует, деля пути по `/`. Так же устроено в продукте (`vmssubsystem/vmsworker/vms`).

Тип — обычный `list`, не отдельный: платформе незачем знать, что элемент — путь. И у него одно ограничение списка (урок 9 М10A): хранилище держит список через запятую, поэтому в уровне не может быть запятой — `Корпус 1, этаж 2` вернётся двумя путями. В продукте ограничение то же.

По дереву **ничего не наследуется**. Папка группирует, но не несёт политики — ни хранения, ни размещения, ни прав. Когда папка должна будет что-то нести, это отдельное решение со своими правилами о том, что ребёнок переопределяет.
