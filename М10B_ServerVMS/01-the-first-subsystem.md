...
```yaml
  fields:
    ...
    folders:               {type: string}                     # the camera's folder, a path: "Объект А/Подъезд"
...
**`folders`** — папка камеры, путь через `/`: `Объект А/Подъезд`. Отдельный ключ, и не ради аккуратности.
...
Папки — дерево, и это **обычная строка**, а не отдельный тип. Платформе незачем знать, что это путь: дерево — это то, что его рисует, деля по `/`. И у строки нет ограничений списка — список хранится через запятую (урок 9 М10A: «элемент не может содержать запятую»), а строка — как написана, так что `Корпус 1, этаж 2` остаётся одним уровнем. Камера лежит в одной папке.
