...
```yaml
  fields:
    ...
    folders:               {type: list}                       # the operator's grouping: "Подъезд,Улица"
...
**`folders`** — папки оператора: «Подъезд», «Улица», «Объект А». Отдельный ключ, и не ради аккуратности.
...
Список плоский. Вложенность здесь не решена: разделитель был бы решением — про пустой сегмент, про ведущий слэш и про то, наследуется ли что-нибудь по дереву, — и принимать его стоит вместе с экраном, который папки показывает.
