...
- **М10A, урок 11** — `EventLog` и `unit_dir`: дерево событий, к которому это дерево пристраивается.
- **М10A, урок 13** — `Resource` и `register`: куда в уроке 8 встанет `ArchivePolicy`.
