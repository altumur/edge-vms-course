...
- **М10A, урок 14** — бакеты: куда попадёт то, что элемент объявит на шине.
...
[**Урок 10**](10-vmsrecorder.md) пишет `rec.subsystem.yaml` и `RecWorker` — четвёртую подсистему и первого носителя `near: vms`: `source` из чужого heartbeat'а, переподписка при переезде держателя, перенос закрытого из спула на старте. Один урок на подсистему — вместо четырёх, которые стоила первая.
