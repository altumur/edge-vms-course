...
[**Урок 10**](10-recworker.md) пишет `rec.subsystem.yaml` и `RecWorker` — четвёртую подсистему и первого носителя `near: vms`: `source` из чужого heartbeat'а, переподписка при переезде держателя, перенос закрытого из спула на старте. Один урок на подсистему — вместо четырёх, которые стоила первая.
