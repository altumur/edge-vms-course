# Lesson 8 — Detectors: the Fourth Subsystem
...
Four subsystems on one box, through one platform, one console, and six units. [**Lesson 9**](09-on-the-box.md) puts all of it on the box М9 built — Quadlet units over one image, everything that must survive an OS update under `/data`, and the health check that rolls an update back when footage stops.
