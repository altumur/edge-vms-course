...
- М9's health check tries the VMS first: a live worker, a camera recording when one is configured, a segment promoted within two segment lengths.
...
One box runs the platform's shape: two stores, a resource, one controller and a console per subsystem, and four subsystems to prove none of them is special. [**М11 — ClusterVMS**](../М11_ClusterVMS/README.md) puts a scheduler under the same processes: the stores become raft and an object store, the units become jobs, and a server dying moves a worker.
