...
One box now runs exactly М11's set of processes: a worker and a controller per subsystem — the recorder's among them — one console, and one resource process with the event database in it. [**Lesson 11**](11-on-the-box.md) puts all of it on the box М9 built as Quadlet units, `resource.container` among them, and М11 turns the same units into jobs with a server dying between them.
