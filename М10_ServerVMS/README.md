...
[`vmsserver-go/`](vmsserver-go/README.md) is the same module in Go — the platform with a test that greps it for the word *camera*, the VMS as a subsystem, 41 tests race-detector clean — and the base [М11's Go port](../М11_ClusterVMS/clustervms-go/README.md) imports, as `clustervms/` imports `vmsserver/`.
