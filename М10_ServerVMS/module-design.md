...
[М9](../М9_EdgeVMS/README.md) ends with a box that owns its OS and its truth: an A/B root under RAUC, and a recorder whose Postgres holds what it should be while a worker makes it so. [М11](../М11_ClusterVMS/module-design.md) then took that recorder, put it under a scheduler, and — in the decision it reached last — took it apart: **workers, resources, one controller** (*2c*). This module is where that shape is built for the first time, on one box, so that every piece can be seen running before М11 spreads it across servers.

> **What changed since М9's recorder** ([М9's design record, Part 2](../М9_EdgeVMS/module-design.md#part-2--the-recorder-lessons-59), Lessons 5–9 of М9). That design gave the recorder its own Postgres, a worker supervising fifty pipelines, and a spool publishing to KVS. Three things are retired here and one is kept. **KVS is gone**: the archive is ours, on the spool's discipline. **The per-box Postgres is gone**: configuration lives in the platform's stores and has one writer. **The worker is gone**: the thing that holds the pipeline is the worker, and it supervises itself. What is kept, unchanged and enforced by the same tests, is the contract: desired persisted and actual derived, `>=` on the revision, backoff with jitter, positions apart from reasons, the epoch in the path, commit-then-publish, the heartbeat carrying its status.
...
| The controller as data                  | **One `SpecController` in the platform, run from a subsystem's spec — `vms.subsystem.yaml`** *(12 September 2026)*. The spec names the rows, how a unit is identified (numbered, or by a field), the operator's fields with types and defaults, derived rows (`vms/retention/<id>`), capacity and headroom as heartbeat fields, a constraint and a tie-break **by name** from a catalogue of two, and the snapshot's fields | Every line of `vmscontroller` was parameterised by names and numbers, not by video: the counter subsystem needed the same class with a different prefix and got it as ten lines of YAML. The line held: YAML declares, it never computes — a rule the catalogue lacks is registered as a function under a name. One process per subsystem still: a generic binary, many jobs, each with its own prefix, token and `count = 1`. |
| The console as data                     | **One `SpecConsole` in the platform, run from the same spec** *(14 September 2026)*. Its own process with its own token: the operator's rows (`vms/cameras/*`, `vms/next_id`, `vms/retention/*`) and never placement — `acl_console()` and `acl_controller()` both derived from the spec. It serves the page, `/spec`, `/<rows>`, `/where`, `/metrics` under the subsystem's prefix, `/marks` and the three writes with the spec's refusals; the page builds its list and its forms from `/spec` and names no camera. A subsystem registers what is its own as extra routes — the VMS, `/timeline` and `/segment` | The console needed exactly what the controller's spec already said — rows, id rule, fields, what counts as running — and nothing about video; the counter subsystem got a console with no console code. `count = 2` because a person waits on it, `count = 1` for the controller because nobody does; neither number is a correctness claim, CAS is |
| What is a subsystem, and what is not    | **Four rows, by three questions — does it have a process that moves, a unit that is placed, a capacity the worker measures?** *(14 September 2026)*. A **subsystem** answers yes to all three: a movable stateless worker (`count = N`, a slot by CAS), units the controller places on it, a capacity in its own number — `vms` (cameras), `live` (viewers), `counter`, М11's `det`. A **resource** cannot move: the archive on a server's disks, a GPU, a VLAN NIC — pinned by physics, heartbeating under `platform/resources/*`, with subsystems *registering* their passes on it; never placed, never a unit. A **data shape on a resource** has no process of its own: events, manifests, buckets — written by whoever holds the unit's epoch, into `<sub>/<unit>/e<epoch>/…`, retained by `<sub>/retention/<unit>`, mirrored by the resource. A **stateless job with no units** needs no controller: the console, the eventindex, the autoscaler | Modelling the archive as a subsystem would give a disk a placement row and let `redistribute()` try to move it when its slot lapsed — the CSI-volume failure the М11 record rejects (Nomad #12118). Modelling events as a subsystem would put a second writer on camera 7 needing an epoch for a unit it does not run — the two-writers problem reintroduced; the bucket is fenced because its writer *is* the segment's writer. What the two need generically they already have: the resource job and its policies, `EventLog`, retention, the mirror, the index as a rebuildable cache. The operator's marks follow the same rule from the other side: the console holds no camera's epoch, so it writes as itself, under `console/<instance>/` |
| Live video: the third subsystem         | **A gateway is a worker whose unit is a camera's fan-out and whose capacity is viewers** *(14 September 2026)*: `live.subsystem.yaml` (rows `streams`, named by camera, `labels` for where viewers are, `grace`), the platform's `SpecController` and `SpecConsole` run from it, `LiveGateway` extending the platform's `Worker`. The unit is created by the console on the first viewer's WHEP offer and deleted by the gateway `grace` seconds after the last viewer leaves; placed by viewer headroom and `labels-subset`; one subscription to the VMS worker's RTP tee (`live_port` in its heartbeat, `20000 + id`) per camera, N `webrtcbin` peers behind it. The worker's side is a leaky branch to `udpsink 127.0.0.1` — the same bytes, no second encode. Media path in `gstvms/webrtc.py` (bench); signalling, placement, fan-out and failover tested with a fake peer | Live view is per-viewer, per-frame work and may not run in the recorder; once it is its own process the platform's question is *what is its unit and what is its capacity counted in*, and answering that made it a subsystem, not a feature — the third YAML, with a unit that is demanded rather than configured and a capacity that is not cameras. `count = N` on viewer headroom is the autoscaler policy the VMS workers already have. No transcoding (a GPU is a placement label, not a default) and no TURN here: those are what М12 adds |
| What runs on the box                    | **М9's box, four Quadlet units over one image** *(14 September 2026)*: `vmsworker@.container` (the slot is the instance name), `vmscontroller.container`, `vmsconsole.container`, `vms-archive-retain.container` on a timer, `vmsgateway@.container`, `vmslivecontroller.container`; `localhost/vmsserver:latest`, which М11's image builds `FROM`; everything that must outlive an OS update under `/data`; М9's `rauc-health-check` reads this worker's archive for *footage is actually being written* | М10 replaced М9's worker and dropped its Postgres without touching the appliance layer, so the claim that it runs on the box was a sentence. Now it is units the tests read (`test_deploy_units.py`: each `Exec=` is a real entrypoint, the mounts agree with the ACL — the controller has no archive, the console cannot write the spool), and the rollback decision reaches the new worker |
...
**М9 gave the recorder its own Postgres so that an operator could edit a camera with everything above the recorder unreachable.** That argument was about the *domain*: М12 may be down, and the recorder must still accept an edit. It said nothing about a writer *inside the cluster*. Under 2c the worker is stateless and the platform's stores are the cluster's; a single writer of configuration inside the cluster — the controller — keeps every property М9 wanted (edits work with the domain gone; the store is one raft, so edits are consistent) and drops the one М9 paid for it (a database per process, migrated at boot on a box nobody visits).
...
5. ~~Could the controller be common to every subsystem?~~ — **Yes, and it is (12 September 2026): the platform's `SpecController`, run from a spec per subsystem** — row *The controller as data*. What stays per subsystem is the worker, a constraint the catalogue lacks, as a named function — and, since the console is `SpecConsole` over the same spec (row *The console as data*), the routes only that subsystem can answer, registered as extras.
...
- [М9's design record, Part 2](../М9_EdgeVMS/module-design.md#part-2--the-recorder-lessons-59) — М9's recorder design this one supersedes, and М9 Lessons 5–9, whose tests are the contract
- [`worker-and-process-model.md`](../М9_EdgeVMS/worker-and-process-model.md) — the tier model this module finally builds
