...
5. ~~Could the controller be common to every subsystem?~~ — **Yes, and it is (12 September 2026): the platform's `SpecController`, run from a spec per subsystem** — row *The controller as data*. What stays per subsystem is the worker, and a constraint the catalogue lacks, as a named function.
