...
| What runs on the box                    | **М9's box, four Quadlet units over one image** *(14 September 2026)*: `vmsworker@.container` (the slot is the instance name), `vmscontroller.container`, `vmsconsole.container`, `vms-archive-retain.container` on a timer; `localhost/vmsserver:latest`, which М11's image builds `FROM`; everything that must outlive an OS update under `/data`; М9's `rauc-health-check` reads this worker's archive for *footage is actually being written* | М10 replaced М9's worker and dropped its Postgres without touching the appliance layer, so the claim that it runs on the box was a sentence. Now it is units the tests read (`test_deploy_units.py`: each `Exec=` is a real entrypoint, the mounts agree with the ACL — the controller has no archive, the console cannot write the spool), and the rollback decision reaches the new worker |
