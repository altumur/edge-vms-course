...
vmsserver/
  psimplatform/                 the platform (named so because Python owns `platform`)
    ...
    spec.py                    Lesson 6  the controller as data: SubsystemSpec (rows, fields, derived rows, placement by name, snapshot, the two ACLs) and SpecController, the one controller every subsystem runs
  ...
  vms/                         the VMS — the first subsystem
    ...
    rec.subsystem.yaml         Lesson 5  the FOURTH subsystem, as a spec: recordings named by camera, retention_days, requires: resource, servers: distinct — the only one placed on the archive
    recorder.py                Lesson 5  RecWorker: VmsWorker over rec/recordings/*, fed by the worker's fan-out from the VMS heartbeat; writes rec/<cam>/e<epoch>/ on its server's archive; re-subscribes when the camera's holder moves; promotes closed segments
  ...
  gstvms/                      Track 2 — needs GStreamer
    ...
    actuator.py                Lesson 4  GstActuator: driverpacksrc ! h264parse ! watchdog ! tee ! rtph264pay ! udpsink, per camera, published by FanOut; the bus drained into (dead, posted)
                               Lesson 5  GstRecActuator: rtspsrc location=<live_url> ! rtph264depay ! h264parse ! watchdog ! archivesink — the recorder's pipeline
...
| 5 | *Record* pressed; a segment in `rec/1/e1/`; the worker moved and the recorder re-subscribed under `e2`; *Stop recording* | `test_lesson5_recorder.py` — the recording placed on the archive (`requires: resource`, `servers: distinct`) and fed by the worker's fan-out under the recorder's epoch, the segment promoted under `rec/` while the worker's tree stays events-only, the re-subscription on failover, the recording that waits while nobody holds the camera, the toggle turned off |
