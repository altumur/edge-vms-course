...
# ================================================================================================
# # check-quadlet.sh — validate the Quadlet units in this directory without deploying them (М9 Lesson 4, Step
# 3)
#
# **Role.** A CI/bench check for the `.container` files beside it. `systemd-analyze verify` cannot check them:
# it does not know the `[Container]` section and ignores the file. The Podman Quadlet generator's dry run is
# the tool — it parses every unit in a directory and prints the generated `.service` units, failing on
# anything it cannot translate. Runs on the bench or in CI (`deploy/check-quadlet.sh`); needs podman's
...
#
# ## Environment
# - `QUADLET_UNIT_DIRS` — set by the script to the deploy directory; overrides the generator's default search
#   path.
# ================================================================================================
set -e
DIR="$(cd "$(dirname "$0")" && pwd)"
