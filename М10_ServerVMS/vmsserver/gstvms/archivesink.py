...
# (`GstActuator` puts `cam["epoch"]` into the launch string), so it is in every path this element writes and a
# zombie's footage is in its own `e<n>` directory. This is "the archive as a resource, from the pipeline's
...
#
# ### `__init__(self)`
# Defaults in `props_`; creates `splitmuxsink` with `muxer-factory=mp4mux` and `async-finalize=True` (the
# closed fragment is finalized on a separate thread so the next one opens without a gap); adds it, ghosts its
# `video` request pad as this bin's `sink`; connects `format-location` to `_location`. `resource` stays `None`
# until the first fragment asks for a location.
#
# ### `do_get_property(self, prop)` / `do_set_property(self, prop, value)`
# Read and write `props_` by property name. Setting `segment-seconds` also sets splitmuxsink's `max-size-time`
# to that many seconds in nanoseconds — the segment length. The other properties are only read when a location
# is asked for, so they must be set before the pipeline starts (the actuator's launch string sets all five).
#
# ### `_location(self, mux, fragment_id)`
# splitmuxsink's `format-location` callback: creates the `ArchiveResource(spool, archive)` on first use (which
# creates both roots), takes the current UTC time to the second as the segment start, and returns
# `segment_path(spool, camera, epoch, start)` after creating its directory. So segments are named by
# wall-clock start, and `vms.archive.parse` can read camera, epoch and start back from the path. `fragment_id`
# is not used: the timestamp is the name.
#
...
# `promoted`. Any exception (the archive unreachable, a different filesystem failing to copy) is logged as a
# GStreamer warning and the segment stays in the spool — where `ArchiveResource.closed_in_spool` finds it on
# the worker's next start and promotes it then. The message is then passed to `Gst.Bin.do_handle_message` so
# it still reaches the pipeline's bus (where `GstActuator._posted` filters it out as plumbing).
#
# ## Notes
# - Ordering, as the archive's docstring states it: a closed segment is promoted (moved, then indexed) and the
#   spool copy is gone only after that. This element never writes the manifest itself and never deletes
#   anything.
# - On an orderly stop the actuator sends EOS before `NULL`, which makes splitmuxsink close the open fragment
#   and post `fragment-closed`, so the last segment is promoted; a `kill -9` loses exactly that open segment
#   (`vmsworker@.container`'s comment on `StopTimeout`).
# ================================================================================================
