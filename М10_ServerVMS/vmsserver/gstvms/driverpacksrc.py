...
# `driverpack://file/<name>` plays `<MEDIA_DIR>/<name>`, paced by its own timestamps, and on EOS seeks back to
# zero while carrying a running offset forward so the pipeline's running time never goes backwards. The
...
# output. `__gstmetadata__` names it "DriverPack source", class `Source/Video`. `__gproperties__` declares one
# read/write string property, `uri`. State: `uri`, the four child elements (`src`, `demux`, `parse`, `pace`),
# `offset` (nanoseconds accumulated across loops) and `last_pts` (the last rebased PTS seen).
#
# ### `__init__(self)`
# Creates and adds the four elements; links `filesrc → qtdemux` statically, `h264parse → identity` statically,
# and `qtdemux → h264parse` dynamically on `pad-added` (a demuxer's pads appear once it has read the file).
# `identity sync=True` is the pacing: a file has no clock, so buffers are held until the pipeline clock
# reaches their PTS — this is what makes a file behave like a live camera and gives `archivesink` real
# ten-minute segments. Adds two pad probes on the identity's src pad: a BUFFER probe (`_rebase`) and a
# downstream EVENT probe (`_on_event`).
#
# ### `do_get_property(self, prop)` / `do_set_property(self, prop, value)`
# The one property. Setting `uri` stores it and sets `filesrc.location` to `uri.resolve(value)` — so a vendor
# URI or a bad name raises `ValueError` from the property setter, which `Gst.parse_launch` in the actuator
# turns into a failed start (logged, `False` returned, the reconciler backs off).
#
# ### `_on_pad(self, demux, pad)`
# Links the demuxer's pad to `h264parse` only if its caps start with `video/x-h264`; audio or other tracks are
# ignored.
#
# ### `_rebase(self, pad, info)`
# The BUFFER probe: for every buffer with a valid PTS, add `offset`, set DTS equal to the new PTS, remember it
# as `last_pts`. Returns `OK` (the buffer passes). On the first loop `offset` is 0 and buffers are untouched.
#
# ### `_on_event(self, pad, info)`
# The downstream EVENT probe: on EOS, do not let it through — set `offset = last_pts + 1/25 s` (one frame past
# the last buffer, so the next file start lands strictly after it), flush-seek `filesrc` back to 0 on a key
# unit, and `DROP` the event. Every other event passes. Because the EOS never reaches downstream,
# `splitmuxsink` keeps its segment open across the loop and `watchdog` sees no gap; because `offset` grows by
...
# - Nothing here knows about cameras, epochs or the archive: the element is a source. The epoch is a property
#   of `archivesink`, set by the worker.
# ================================================================================================
