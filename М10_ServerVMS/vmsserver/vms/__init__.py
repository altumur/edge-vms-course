...
# the platform (`vmsplatform/`) was built to host without knowing what it is — and gives a one-line map of the
# package: `reconciler.py` (М9 Lesson 6's loop, unchanged: desired persisted, actual derived), `archive.py`
# (the archive as a resource: spool → promote → manifest; retention as a policy), `worker.py` (vmsworker —
# DriverPack as the worker, N pipelines against an assignment), `controller.py` (vmscontroller — the only
# writer of `vms/*`: cameras, assignment, placement) and `console.py` (the one-box console: the platform's
# `SpecConsole` over the VMS spec, plus `/timeline` and `/segment`).
#
# The last sentence fixes the dependency direction that
# `tests/test_lesson1_platform.py::test_the_platform_knows_nothing_about_video` enforces: `vms/` imports from
# the platform only through its public interfaces (`Worker`, `SpecController`, `SpecConsole`, `EventLog`, the
# store Protocols), and nothing in the platform imports from here.
#
# ## Module-level names
...
#
# ## Notes
# - The docstring still says `platform/` where the directory is `vmsplatform/` (renamed because Python's
#   standard library owns `platform`; see `vmsplatform/__init__.py`).
