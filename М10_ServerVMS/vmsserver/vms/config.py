...
# conversions the worker and the tests need — a Variables row of strings to a typed camera dict, and back. The
# docstring is the VMS's key map, worth keeping in view:
#
...
# - `vms/placement/<id>` — `worker, reason, at, rev` (the controller writes).
# - `vms/retention/<id>` — `days`, derived from `events_retention_days` (the controller writes; the platform's
#   resource job reads).
# - `vms/epoch/<id>` — `epoch` (a worker takes, by CAS).
# - `vms/next_id` — `n` (the controller).
#
# "A camera row is small, rare and must be consistent: raft's shape. Nothing here is controller-derived status
# — that is in the worker's heartbeat." Used by `worker.py` (`row` on every refresh), `controller.py`
...
# - `FORBIDDEN_FIELDS` — an alias of `vmsplatform.spec.PLATFORM_FIELDS` (`worker, placement, epoch, revision,
#   observed_revision, phase, id`): what `SubsystemSpec.refuse` rejects in a create/update body. Kept under
#   the VMS's old name for readers of earlier lessons.
#
# ## Functions
#
# ### `row(items) -> dict`
# `SPEC.row(items)`: Variables items (all strings) to a typed dict with `id`, every spec field (its default if
# absent) and `revision` (default 1). The worker calls it on each camera row named by its assignment; `retain`
# calls it on every row under `vms/cameras/`.
#
# ### `items(row_) -> dict`
# `SPEC.items(row_)`: the inverse, everything as strings (`bool` → `"true"/"false"`, lists comma-joined). The
# controller uses the spec's method directly; this wrapper exists for symmetry and for the tests.
#
# ## Notes
# - Neither function filters the `deleted` marker: a row the controller marked `deleted: "true"` converts like
#   any other. `VmsWorker.refresh` checks the marker itself before calling `row`; `__main__.retain` does not.
