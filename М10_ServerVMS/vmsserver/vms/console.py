...
# ================================================================================================
# # console.py — the one-box console: the platform's SpecConsole over the VMS spec, plus the two media routes
# only a VMS has
#
...
# `vms.subsystem.yaml` (see `vmsplatform/console.py`); nothing in this file knows what a camera's fields are.
# What the VMS adds is the bytes: `GET /timeline/<id>?from&to` (segments and event buckets from this box's
# archive manifest, fenced ones marked) and `GET /segment/<path>` (one promoted segment, `Range` honoured, for
# the page's `<video>`). They are *registered* as the console's `extra` route function, not subclassed. The
# console is its own process (`python3 -m vms console`, `deploy/vmsconsole.container`) with its own token —
# `vms/cameras/*`, `vms/next_id`, `vms/retention/*`, `vms/idem/*` — and never placement. Depends on
# `archive.py` (`ArchiveResource`, `Manifest`) and `controller.py`.
#
# ## Module-level names
# - `PAGE`, `send_file` — re-exported from `vmsplatform.console` (`noqa: F401`) for М11, which serves the same
#   page and the same ranged file replies from a Nomad job.
#
# ## Functions
#
...
#
# - `GET /segment/<rel>` — `rel` is joined under `archive.root`. If it contains `..` or is not a regular file
#   the reply is `404 {detail, error: "no such segment"}`. Otherwise `send_file(handler, path, "video/mp4")`
#   writes the reply itself (whole, or `206` with `Content-Range` when the request carried `Range`) and
#   `extra` returns `()` — the console's signal that the reply was already served. The test asks `bytes=10-19`
#   of a 256-byte segment and gets exactly those bytes with `Content-Range: bytes 10-19/256`; a path that does
#   not exist is 404.
# - `GET /timeline/<id>?from&to` — `int` of the last path segment; `200` with `Manifest(archive.root,
#   cid).timeline(from, to)` — `from` defaults to 0, `to` to `1e12`. Note `current_epoch` is not passed here,
#   so on one box no span is marked `fenced` by this route; the `Manifest.timeline` fencing is exercised
#   directly in `test_lesson3_archive.py`.
...
# media=archive is not None)`. With an archive: operator marks (`POST /marks`) go into the console's own event
# log under `console/<hostname:pid>/e1/` on this server's resource, and `/spec` reports `media: true` so the
# page draws a timeline and a player. Without one: no marks (503) and no media.
#
...
#   `console/<unit>/e1/…` and `subsystems_under(archive)` shows only `console` — never `vms/1/`, whose bucket
#   has one writer; the page never says "camera" outside its HTML comment; then `/timeline/1`, a ranged
#   `/segment/`, a 404, a PUT that bumps `revision` to 2, and a DELETE whose placement waits for
#   `unplace_deleted`.
# - The archive mount in `vmsconsole.container` is what lets `/segment/` serve bytes; `/data/spool` is mounted
#   read-only there because the console reads and never records.
# ================================================================================================
