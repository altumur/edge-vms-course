...
# ================================================================================================
# # worker.py — vmsworker: DriverPack as the worker — N pipelines against an assignment, an epoch per camera,
...
# `NOMAD_ALLOC_INDEX` (the slot preference; the CAS claim on `vms/slots/w-N` is the proof), `NOMAD_NODE_NAME`
# or the hostname (`server`: which resource it records into), `NOMAD_META_labels` (`labels`: what this server
# can reach; the controller places by them), `NOMAD_ALLOC_ID` (the instance), `CAPACITY` (the worker's own
# number, from М9 Lesson 7's probe). Run by `__main__.worker`; tested in `tests/test_lesson4_worker.py` and
# used across Lesson 5's tests.
#
# ## Module-level names
...
# to. It mirrors `GstActuator`'s surface: callable, `pump()`, `stop_all()`, plus test hooks. State: `failing`
# (a set of camera ids, or a predicate, whose start fails), `calls` (every `(verb, id)`), `running` (ids
# started and not stopped), `epochs` (`{id: epoch}` as passed in the row at start), `dead` and `posted` (what
# tests push in to simulate a bus).
#
# ### `__init__(self, failing=frozenset())`
# ### `__call__(self, verb, cam) -> bool`
# Records the call. `stop` always succeeds and removes the id. A start/restart on a failing id fails (and
# clears `running`); otherwise the id is running and `cam["epoch"]` is remembered — the tests read
...
# even from a holder that has not lapsed, because the scheduler is the authority on which process is current;
# `None` means the environment's, and failing that "whichever slot is free" — a lapsed one first, so a
# replacement inherits its assignment. "A worker on a cluster is a worker on a box whose stores happen to be
# raft: same class, same heartbeat." It is also the `Store` of its own `Reconciler` (`desired()`).
#
...
# archive_root=None, bucket_seconds=600, env=None)`
# `env` defaults to `os.environ` (tests pass a dict). `instance` defaults to `NOMAD_ALLOC_ID`, else the base
# class's `hostname:pid:6hex`. Calls `Worker.__init__` with `name=None` and then `claim_slot(prefer=name or
# slot_from_environment(env))` — so construction *is* the claim, and `self.name` is set afterwards. Then:
# `archive_root` from the argument or `$ARCHIVE` (`/data/archive`); `capacity` from the argument or
# `$CAPACITY` (50) — "М9 Lesson 7's B + n·I, measured on ITS server"; the actuator (`FakeActuator()` if none);
# an empty `rows`; the `Reconciler(self, self._actuate)`; `recording_allowed = True`; `server` from the
# argument, `NOMAD_NODE_NAME`, `NOMAD_NODE_ID`, else the hostname; `labels`, `alloc`; the two start clocks.
# Finally it reads the previous heartbeat object of this slot name: if one exists and was written by a
# different instance, `previous_hb` is its `ts` and `previous_instance` its instance — the controller's
# `failover_seconds` computes `started − previous_hb` from these, measured from what the workers wrote.
...
# `vms/cameras/<id>`; rows that are missing or marked `deleted: "true"` are skipped. "A fresh worker knows
# nothing and reads everything; nothing about what is running is stored." An unassigned worker has no rows and
# invents nothing (`test_worker_runs_its_assignment…`: the first `reconcile_once` is `[]`).
#
# ### `_actuate(self, verb, cam) -> bool`
# The gate between the reconciler and the real actuator. For `start`/`restart`: refuse if the instance is
# fenced (`recording_allowed` false); on `start`, or if no epoch is held for the unit, `take_epoch(unit)` — a
# new epoch for a new writer — else reuse the held epoch (an edit's restart keeps epoch 1); refuse if
# `may_write(unit)` is false (no lease, or a lost one); then call the actuator with `epoch` added to the row —
# the number archivesink puts in every path. For `stop`: call the actuator and `release(unit)` (forget epoch
# and lease). `test_lease_expiry_without_renewal_stops_starts`: after 26 s without renewal `may_write` is
# false; a later start takes epoch 2.
#
# ### `now(self) -> float`
# Seconds since start on the monotonic clock — the reconciler's `now` for backoff.
#
# ### `reconcile_once(self, now=None) -> list[(verb, id)]`
# One pass: `refresh`, `reconciler.reconcile(now)`, count the pass, log each action. The base class's abstract
# method; called by `run` and directly by every test.
#
# ### `lease_pass(self) -> list[str]`
# Renew the slot and every lease, and decide what a lost lease means. First `renew_slot()`: if the slot is
# held by another instance now, `fence("slot w-N is held by another instance now")` and return every held unit
# — the zombie is fenced at the slot *before* any epoch is looked at
# (`test_the_zombie_is_fenced_at_the_slot_first`: the replacement's `slot.gen == 2`). Then `renew_leases()`;
# for each lost unit: if it is no longer in my assignment this is a reassignment — stop the pipeline, drop it
# from `reconciler.actual`, `release` it, and carry on recording the rest
# (`test_a_reassignment_is_not_a_zombie`: released, not fenced, `recording_allowed` still true); if it *is*
# still mine, another instance of me took the epoch — I am the zombie — `fence` and stop looking. Returns the
# lost units. `test_the_zombie_on_one_box`: A's `lease_pass` returns `["1"]`, A is fenced with "slot w-1" in
# the reason, its epoch lease also reports a conflict, and it may start nothing (`("failed", 1)`); B is fine.
#
# ### `fence(self, why)`
# Once: log at error, set `recording_allowed = False` and `fenced_reason`, `actuator.stop_all()`,
# `reconciler.clear()` — the pipelines were stopped underneath the loop. Idempotent (a second call returns
# immediately). After this the heartbeat says `fenced: true`, `_actuate` refuses every start, and `observe`
# writes nothing.
#
# ### `observe(self, cid, kind, **fields) -> str | None`
# An event: written by this worker, now (`wall()`), into the camera's bucket on this server's resource under
# the epoch this worker holds for it — recording or not. `None` if no epoch is held for the camera (not mine
# to observe) or the instance is fenced. Records `(cid, t, kind)` in `observed` and returns the bucket path
# from `event_log(archive_root, cid, epoch, bucket_seconds).append(...)`. Nothing else is told — no store
# write, no controller. `test_the_worker_observes_what_it_holds_recording_or_not`: before the first reconcile
# `observe(1, ...)` is `None`; after it the line lands in `<archive>/vms/1/e1/…`; camera 2 (not assigned) is
# `None`; after `fence` a post is dropped; `vms/events` in the store stays empty.
#
# ### `pump_once(self)`
# The bus, drained: `actuator.pump()` gives `(dead, posted)`; every posted `(cid, kind, fields)` becomes
# `observe(...)` — a line only if I still hold the epoch; every dead camera becomes `reconciler.lost(cid,
# now)` (restart after backoff) plus `observe(cid, "silent")` — "the event with no segment open, by
# definition".
#
...
# `position` (`converged | lagging | stalled`), `revision`, `observed_revision` (what is actually running),
# `epoch` (held, or 0). This list is the heartbeat's `status`; the controller's `read_model` and the console's
# `/cameras` show it, and `/metrics` counts `phase == running` into `vms_cameras_recording`.
#
# ### `headroom(self) -> int`
# `max(0, capacity − len(rows))`: cameras this worker could still take. "Not CPU — a worker at 40 % CPU with
# no assignment left is full." What the autoscaler reads via the controller's `headroom()` and `/metrics`.
#
# ### `heartbeat_once(self)`
# `Worker.heartbeat(status, …)` to the object `vms/<name>/heartbeat` with the extras the platform reads by
# name: `server`, `instance`, `alloc`, `labels` (comma-joined), `assignment_rev`, `fenced`, `conflicts`,
# `passes`, `capacity`, `headroom`, `started`, `previous_hb`, `previous_instance`. The controller's
# `capacity_of`, `labels_of`, `server_of`, `headroom`, `failover_seconds` and the console's metrics all read
# from here.
#
...
# `vmsworker@.container` gives it `StopTimeout=20`), a last heartbeat, then `release_slot()` — "an orderly
# stop says so; a crash says nothing", which is what lets the controller tell scale-in (redistribute) from a
# crash (leave it to the scheduler).
#
# ## Notes
# - Ordering: the slot is claimed in the constructor, before any assignment is read (the name is the row key);
#   an epoch is taken in `_actuate` before the pipeline starts; `lease_pass` checks the slot before the
#   leases.
# - Recovery needs no controller: `test_restart_with_the_controller_stopped` deletes the controller, starts a
#   fresh `w-1` with an empty `actual`, and it starts all three cameras from its assignment with epoch 2 each
#   — the old instance is fenced by construction.
# - Three exits from a lost lease, all in `lease_pass`: reassignment (stop that one, continue), zombie (fence
#   everything), and slot taken (fence everything, first). A lease that merely expired because the loop
#   stalled shows up as `may_write` false in `_actuate` and a fresh epoch on the next start.
