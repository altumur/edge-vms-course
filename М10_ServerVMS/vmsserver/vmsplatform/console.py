...
# units are, which fields the operator owns and what leaves the cluster; that is everything a console needs to
# list, edit and show them, so the console is one class run from the same spec. It serves `console.html`,
# `/spec` (what the page reads first), the rows with the read model, `/where`, `/resources`, `/unplaceable`,
# `/events` (if an `EventIndex` is behind it), `/metrics`, and the writes — POST/PUT/DELETE on the rows and
# POST `/marks` — with idempotency keys stored in Variables so a retry answered by another console instance is
# the same request. What a subsystem adds is registered, not subclassed: `extra(handler, method, path, query)`
# gets every request the built-in routes do not claim (the VMS: `/timeline` and `/segment`). The console holds
# the subsystem's `SpecController` with the *console's* token (the operator's rows, never placement), so a
# write it should not make is a 403 from the store, not a rule in this file. `vms/console.py` builds it via
# `make_console`; the deploy unit `vmsconsole.container` runs it as its own process.
#
# ## Module-level names
...
# process: `<sub>/idem/<key>` is claimed by a create-only CAS before the write and filled with the reply after
# it. A second instance that sees the claim waits for the reply and serves it; it never repeats the write.
# Keys older than `ttl` are pruned on the way past, at most once a minute.
#
# ### `__init__(self, vars_, prefix, wall, ttl=86400.0, clock=time.monotonic, sleep=time.sleep)`
# `prefix` is `<sub>/idem/`; `wall` stamps `at`; `clock` rate-limits pruning; `sleep` is the wait between
# polls (injected for tests).
#
# ### `_path(self, key) -> str`
# Refuses an empty key, one containing `/` or `..`, or longer than 200 chars (`Refused`: must be one path
# segment) and returns `prefix + key`.
#
# ### `claim(self, key) -> None | (status, body)`
# Try `put({state: pending, at}, cas=0)`: success means ours to answer — return `None` (the caller does the
# write, then `store`). On `Conflict`, another instance holds it: poll up to 40 × 50 ms; if the row vanished
# (pruned or the claimant crashed mid-flight) claim again; if `state == done` return `(status, body)`; after
# the polls, `409 in flight`. `test_a_retry_that_lands_on_another_console_is_one_camera` covers all of it: the
# second console returns the first's 201 body; a key with a pending claim makes console A wait and then serve
...
#
# ### `prune(self) -> int`
# At most once per 60 s of monotonic time: delete every key under the prefix whose `at` is older than `ttl`,
# by CAS (a conflict skips it). The test advances a day and sees 2 pruned, then 0, then a re-used key create a
# new camera — a forgotten key is a new request, by design.
#
# ## `class SpecConsole`
# One console for every subsystem.
#
# ### `__init__(self, ctl, marks_root=None, index=None, worst_failover=0.0, wall=None, extra=None,
# media=False, lost_after=45.0)`
# `ctl` is the subsystem's `SpecController` holding the console's token; `marks_root` is this server's
# resource root — if given, `self.marks` is an `EventLog(marks_root, "console", <hostname:pid>, epoch 1)` (the
# console's own log; one writer, so epoch 1 forever); `index` is an optional `EventIndex`; `worst_failover` is
# a number exported on `/metrics`; `extra` is the subsystem's route function; `media` tells the page it may
# draw a timeline and play. `seen` is the `IdempotencyKeys` over `<sub>/idem/`. `_scan` caches the assignment
# directory; `scans` counts cache refreshes.
#
# ### `describe(self) -> dict`
...
# `<p>_failover_seconds{kind="worst"}`; `<p>_resources_live`; and `<p>_<running_gauge>` — the count of status
# entries in phase `running` on live workers (`vms_cameras_recording`). The page reads two of these for its
# status line.
#
# ### `create(self, body) -> (status, dict)`
# `ctl.create(body)` → 201 with the row plus `worker: None` (placed by the controller's next pass, never by
# the console); `Refused` → 400 `{detail, error}`.
#
...
# An operator's observation. 503 if there is no resource on this server; 400 unless the body names `cam` or
# `unit`. Appends `{kind: mark, user, note, cam|unit}` at `wall()` to the console's own bucket — `cam` is
# stored as an int because it is the field the index joins on — and returns 201 `{subsystem: "console", unit:
# <instance>, bucket: <relative path>}`. It is the console's event, into `console/<instance>/e1/…` on this
# server's resource, never a worker's bucket.
#
# ### `handler(self)`
...
# - `_extra(method, path, q) -> bool` — call the subsystem's `extra`; `None` means not ours (return False).
#   `()` means the extra wrote the reply itself (`send_file`). `(status, dict|list)` is sent as JSON;
#   `(status, bytes)` raw; `(status, bytes, headers)` raw with headers.
# - `do_GET` — routes, in order:
#   - `GET /` or `/index.html` — `console.html`.
#   - `GET /spec` — `describe()`.
#     - `GET /<rows>` — `{rows: ctl.read_model(lost_after), configured: ctl.units()}`: the heartbeats' view
#     over the configured rows.
#     - `GET /where/<id>` — `{worker, reason}` from the stored placement (404 with nulls if unplaced), plus
#     `directory` (the assignments' answer) and `scans`.
#   - `GET /resources` — every resource heartbeat with `state: live | silent` by `lost_after`.
#   - `GET /unplaceable` — `ctl.unplaceable()`.
#     - `GET /events?from&to&cam|unit&kind&subsystem` — 503 if no index; else builds `current_epochs` from
#     every `<sub>/epoch/*` row and calls `index.query`. A numeric `unit` is treated as `cam`; a non-numeric
#     one is passed as `unit`.
#   - `GET /metrics` — `metrics_text()` as `text/plain`.
#   - otherwise `_extra("GET", …)`; then 404 `{detail, error}`.
# - `_idem() -> key | None` — for POST: 400 if `Idempotency-Key` is missing or malformed; if `claim` returns a
#   prior reply, send it and return `None`; else return the key.
# - `do_POST`:
#   - `POST /<rows>` (Idempotency-Key required) — `create(body)`; the reply is stored under the key and sent.
#     - `POST /marks` (Idempotency-Key required; `X-User` header, default `operator`) — `mark(body, user)`;
#     stored and sent.
#   - any other path — `_extra("POST", …)` or 404.
# - `do_PUT`:
#     - `PUT /<rows>/<id>` — `update(uid, body)`; `Idempotency-Key` is optional here: if present the same
#     claim/store dance applies. Any other path is 404.
# - `do_DELETE`:
#     - `DELETE /<rows>/<id>` — `delete(uid)`; no idempotency key (a second delete is 404, "gone is gone").
...
#
# ## Notes
# - `test_the_console_over_http` walks the whole surface: POST twice with one key is one camera; the console's
#   controller cannot `place` (`Forbidden`); PUT `{"worker": "w-9"}` is 400; `/cameras` rows show `phase
#   running` and `server srv-1`; `/where/1` agrees with the directory; `/spec` says `rows cameras, media
#   true`; `/metrics` contains `vms_cameras_recording 1`; `/marks` writes to `console/<instance>/e1/`; the
#   page mentions `/spec`, `/timeline/`, `/segment/`, `<video>` and never the word camera outside its comment;
#   `/timeline/1` and a ranged `/segment/` come from the VMS extra; PUT `{"enabled": false}` bumps revision to
#   2; DELETE marks the row and the placement waits for `unplace_deleted`.
...
# - `/events` relies on the resource's HTTP and an `EventIndex`; on one box without one it is an honest 503,
