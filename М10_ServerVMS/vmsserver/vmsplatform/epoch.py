...
# renewal and stops. This file has the issuer (`next_epoch`), the reader (`current_epoch`) and the lease that
# a holder keeps on its epoch (`Lease`). The subsystem decides the key (`Subsystem.epoch_key(unit)` gives
# `<name>/epoch/<unit>`); the platform promises only that numbers come from one issuer, in order.
...
# Reads the epoch stored under `key`, 0 if the key does not exist. Used by `Lease.renew` and by the console's
# `/events` route to compute the current epoch per unit for fencing in the index's reply.
#
# ## `class Lease`
...
# does not by itself stop writing; the TTL does. Created by `Worker.take_epoch`; `Worker.renew_leases` renews
# all of them and reports the ones lost.
#
# ### `__init__(self, vars_, key, epoch, ttl=30.0, margin=5.0, clock=time.monotonic)`
# `key` is the epoch row, `epoch` the number this holder took, `ttl` and `margin` the window (writes stop
# `margin` seconds before the TTL to leave room for the fence to propagate), `clock` a monotonic clock (tests
# inject a fake). Sets `last_renewal = clock()` now, `fenced = False`, `conflicts = 0`.
#
# ### `renew(self) -> bool`
# Once fenced, always `False`. Otherwise read `current_epoch(key)`: if the store raises (unreachable), do not
# fence — return `may_write()` and keep going until `ttl − margin` runs out; if the live epoch differs from
# mine, set `fenced = True`, count a conflict and return `False`; else stamp `last_renewal` and return `True`.
# `conflicts` is what the worker sums into its heartbeat (`conflicts=`) and the console exports as
# `<sub>_epoch_conflicts`.
#
# ### `may_write(self) -> bool`
...
#
# ## Notes
# - `test_lease_on_a_monotonic_clock`: at 24.9 s the lease may write; at 25.1 s it may not and `seconds_left()
#   == 0`; a successful `renew()` restores it; after someone else calls `next_epoch` on the same key,
#   `renew()` returns False, `fenced` is set and `conflicts == 1`.
# - The fence is discovered at renewal, never pushed: a zombie keeps writing for at most `ttl − margin` after
#   the new holder took the epoch. That bounded window is the RPO the archive lesson accepts, and the epoch in
#   the path is what lets the manifest mark that window as fenced afterwards.
# ================================================================================================
