...
What a cluster adds is search — across cameras, and across subsystems — and `psimplatform/eventindex.py` is that: **one index per resource, none per console, none cluster-wide.** Each resource job runs a `ResourceIndex` over its own tree — a SQLite table it fills by reading its own buckets (and the copies it holds of its peers', below) and that it can throw away — and answers `GET <resource>/events?cam=…` from it. The console holds no index: its `/events` asks every live resource and merges by time, exactly as `/timeline` merges manifests, naming the ones that did not answer and fencing each event by its unit's current epoch, which only the cluster's rows know (`cluster.console.MergedIndex`). Nothing crosses the network to *build* an index; nothing anywhere holds every server's rows. The first draft had one `count = 1` index job reading every resource over HTTP — a cache, but a cluster-wide one, with a rebuild that scaled with the cluster and a copy of every event on one server. The three questions of the design record's *What is a subsystem* row settle where it belongs: the index has nothing to place, so it is not a subsystem; and what it indexes is on a resource, so it lives with the resource. Its two properties are the controller's, in the form that matters for a cache:

```
srv-a: ResourceIndex.rebuild() -> {added: 2, segments: 1}     its own tree, nothing else; srv-c: {added: 2, segments: 2}
...
srv-a back:     merge -> [7, 8]                 state: "live"                        its index answers again; nothing rebuilt
srv-b live but not answering: merge -> [7]      state: "live; srv-b unreachable"
retention on srv-b removed a bucket: srv-b's index forgets it; the merge -> [7]
```
...
knob on:   pol[srv-a].once() -> mirrored: 2, peers: [srv-b]   closed buckets only; the open one is the RPO
           srv-b's index: rebuild -> from_mirror: ['srv-a'], added 3       "live; srv-a from mirror" — its own bucket and srv-a's two copies
           ...
           its index rebuilt: added 2; the merge -> the same two rows, state "live"    the resource is the source again; no duplicates
```
...
- Events are buckets on the resource, per subsystem and unit, written by the worker holding the epoch; each resource's `eventindex` is a cache over its own, the console merges them and holds none, subsystems are joined on a field, and a silent server is named, not guessed; a controller writes none of them, and two servers cannot hold a quorum for anything.
