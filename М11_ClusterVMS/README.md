...
There is no Go port of this module, and there is no longer a reason for one. A worker on a cluster is a worker on a box whose stores happen to be raft, so the Go worker this module runs is [`vmsserver/worker/`](../vmsserver/worker/README.md) unchanged — what is М11's about it is the jobspec, not the binary. The controller, the console and the resource are Python here as they are on a box; [`vmsserver/reference-go/`](../vmsserver/reference-go/README.md) keeps what they looked like in Go.
