...
This module has no Go port of its own any more. A worker on a cluster is a worker on a box whose stores happen to be raft, and the same is true a level up: what is М11's about running on Nomad is the jobspecs and the raft-backed `Variables`, not another copy of the loops. М10's [`vmsserver-go/`](../vmsserver-go/README.md) is whole and current, 89 tests, and a Go port of this module — should one be wanted again — is built **on** it the way `clustervms/` is built on `vmsserver/`, with a `replace` directive instead of `sys.path`.
