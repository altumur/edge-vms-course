...
- The search loop — tries three candidates in order and appends the **first** existing directory to `sys.path`:
  1. `$VMSSERVER_PATH` — set to `/app` by `deploy/Containerfile`, where М10's image already holds `vms/`, `vmsplatform/` and `gstvms/`;
