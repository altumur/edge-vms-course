...
| **Events** | in the dead Postgres | **on the dead resource beside their segments** — unavailable with the footage, not lost; each resource's `eventindex` covers its own tree; the console's merge says which server is silent, and takes its answer again when it returns |
...
- **Events: the database that is a cache.** On the resource beside the segment; `eventindex` on each resource, rebuilt from its tree, merged by the console; why not an event controller over a two-server database; the storage knob's events row
