...
| One controller class, not two | **`ClusterController` and `ClusterWorker` are imports of М10's** *(12 September 2026)* | The first cut subclassed М10's controller to add label constraints, the server in the reason, the snapshot and the measured failover — and every one is the one-box behaviour with N = 1: empty labels, the hostname, a cluster of one. A subclass that adds nothing a box lacks is a teaching device, not a design; and with the controller run from a spec (М10's *The controller as data* row) there is nothing left to subclass. М11 now adds zero controller code, which is a better proof of *a cluster is a box whose stores happen to be raft* than a diff was. |
...
| Language | **Designed in Python, shipped in Go and C++** | The platform, the controller and the worker's loop port to Go — [`vmsserver-go/`](../М10_ServerVMS/vmsserver-go/README.md) and [`clustervms-go/`](clustervms-go/README.md), 70 tests between them, 8.9 MB vs 21.2 MB at idle; the worker's pipeline is DriverPack's C++. The contract is what both keep. |
...
| The event database: one per resource | **`EventDatabase` inside each resource job, over that resource's own tree — its buckets and the mirror copies it holds — served as `GET <resource>/events`; the console holds no database and its `/events` fans out to the live resources and merges by time (`MergedIndex`), fencing by the cluster's epochs, naming the unreachable, dropping a peer's copy when the owner answers** *(14 September 2026)* | The first cut was one `count = 1` index job reading every resource over HTTP: a cache, but a cluster-wide one — a rebuild that scales with the cluster, every server's rows on one server, and a job with nothing to place still needing a home. What it indexes is on a resource, so it lives with the resource; what needs a cluster-wide answer is a *query*, and a query merges, the way `/timeline` already does. No HTTP is crossed to build a database, only to answer a query; a console restarted rebuilds nothing because it holds nothing. On the box the same process runs as `vmsresource.container` and the console asks it — М10 Lesson 9; the job here is М10's `vms_resource` |
...
| On Server A | Under the recorder model | Under 2c |
|---|---|---|
| **Configuration** — cameras, assignment | in the recorder's Postgres; published upward; restored from an object with an RPO | **in raft already** — the controller wrote it there; `w-3` reads it on Server B. Nothing travels; the RPO is zero |
...
| **Events** | in the dead Postgres | **on the dead resource beside their segments** — unavailable with the footage, not lost; each resource's `eventdatabase` covers its own tree; the console's merge says which server is silent, and takes its answer again when it returns |
...
- **Events: the database that is a cache.** On the resource beside the segment; `eventdatabase` on each resource, rebuilt from its tree, merged by the console; why not an event controller over a two-server database; the storage knob's events row
...
4. ~~The rewrite order.~~ — **Done (12 September 2026).** `clustervms/` is the Nomad implementations of `psimplatform` plus the cluster's additions (constraints, the resource's heartbeat and served manifests, the merged timeline, the snapshot); the five lessons follow this record. `clustervms-go/` was ported to 2c the same day, on a new `vmsserver-go/`, with both suites and the measurement redone.
...
6. ~~Why two controllers — `VmsController` and `ClusterController`?~~ — **Merged (12 September 2026)**: row *One controller class, not two*. The same for the worker.
