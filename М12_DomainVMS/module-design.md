...
| The "primary camera" | **The domain cluster, hosted on a camera** | This module already has one designated cluster that hosts the domain services and fails with nothing to fail over to. On a camera it is a domain cluster of one node, so re-hosting it becomes routine rather than a disaster drill — and an old host that comes back is routine too, which needs a **term** (Lesson 14). |
| An edit for a camera that is off | **Kept, per field, and applied when it is back (Lesson 9)** | `503` is honest and, for a member that is one device and often off, useless. Kept beside the grants, carried home by the agent, applied by the camera's own console as the operator, grant checked then; *applied / already / conflict* per field; outcomes matched by version. The owner never changes. |

### Lessons

- **9 — An Edit for a Cluster That Is Off.** *Written.* See the lesson; `domain/pending.py`, `tests/test_lesson9_pending.py`.
- **10 — A Cluster of One.** A unit pinned to hardware (`place: device`), no orchestrator, the camera taking its own epoch, the domain agent as the only link upward.
- **11 — Hundreds of Small Members.** The directory and read view are stated to hold "low hundreds of clusters' worth" (*Several clusters, one domain*). Measure it at that shape before promising it.
- **12 — Shared Settings Without a Database.** Retention defaults, the folder tree, scenarios between cameras: published once and signed — the brief's immutable file is this module's published-object pattern — carried to every member by its agent.
- **13 — A Stream From Another Cluster.** A server's recorder records a camera in a camera's cluster and closes gaps from its card (М10B Lesson 16). It finds the camera's `live_url`, `playback_url` and `coverage` through the directory, not in its own cluster's heartbeats. Data crosses; work does not move.
- **14 — A Domain Cluster of One Node.** Only cameras. The domain on one of them; a term so a returning old host steps down; re-hosting from the backed-up key as an ordinary operation; the kept edits of Lesson 9 added to what the domain must publish beyond itself.

---
## The code, whole

[`domainvms/`](./domainvms/README.md) is the eight lessons as one runnable package, built **on** М11's `clustervms/` and М10's `vmsserver/` — the domain reads each cluster's snapshot and heartbeats through М11's object-store adapters, forwards writes to each cluster's console, and holds no database. In its tests every cluster is the real thing: М11's `ClusterController` placing on М10-shaped `ClusterWorker`s over linked in-memory stores, so what the domain reads is what the cluster actually wrote. Every decision above has a file: `federation.py` (`Cluster.snapshot()/heartbeats()`, and an `Answer` that says what it could not reach, by ref), `placement.py` (reachability, CAS, a dead cluster is not a trigger), `shadow.py` (`WorkerReport`, keyed by ref), `readview.py` and `console.py` (the camera list from heartbeats, one cause per dead server), `api.py` (`ClusterConsole`, the forwarded write, `FORBIDDEN_FIELDS`), `gateway.py` (`WorkerLiveEndpoint`, the tee with a leaky queue, one subscription per camera), `tokens.py` / `signer.py` / `identity.py` / `grants.py` / `agent.py` (the signer, users that never reach a cluster, `ClusterGrants` and `ClusterAuthoriser` with expiry, the agent that carries only trust into `domain/*`), `enroll.py` (voucher and approval), `entitlement.py`, `cloud.py` (`render_three_ways`, the worker stanza that is the same everywhere). Four Nomad jobs and two ACL policies under `deploy/`. Its tests — 54 with Lesson 9's `pending.py` — run with no Nomad, no Postgres and no browser, and each lesson's deliverable is a named test.
