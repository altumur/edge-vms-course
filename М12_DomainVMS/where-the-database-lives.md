...
| **Archive index** — which segment holds which time range | **A cache on the resource** (М10 `Manifest`, per camera per epoch) and a **SQLite cache over every resource** (`psimplatform/eventindex.py`) that can be dropped and rebuilt from the buckets | An index that is never the source of truth is a cache, and a cache that admits to being one needs no durability story. The manifest is beside the segments it names |
